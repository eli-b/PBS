...
Node::Node() : loc(0), g_val(0), h_val(0), parent(NULL), timestep(0),
               num_internal_conf(0), in_openlist(false), num_non_hwy_edges(0)
{
}

Node::Node(int loc, double g_val, double num_of_collisions, Node* parent,
           int timestep, int num_internal_conf, bool in_openlist, int num_non_hwy_edges) :
        loc(loc), g_val(g_val), h_val(num_of_collisions), parent(parent), timestep(timestep),
...
{
    if (n.parent != NULL)
        os << "LOC=" << n.loc << " ; TIMESTEP=" << n.timestep << " ; GVAL=" << n.g_val
