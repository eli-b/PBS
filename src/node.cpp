...
Node::Node() : loc(0), action(MapLoader::WAIT_ACTION), g_val(0), h_val(0), parent(NULL), timestep(0),
               num_internal_conf(0), in_openlist(false), num_non_hwy_edges(0), orientation(MapLoader::FACE_EAST)
{
}

Node::Node(int loc, MapLoader::valid_actions_t action, double g_val, double num_of_collisions, Node* parent,
           int timestep, int num_internal_conf, bool in_openlist, int num_non_hwy_edges,
...
{
    if (n.parent != NULL)
        os << "LOC=" << n.loc << " ; ORT=" << n.orientation << " ; TIMESTEP=" << n.timestep << " ; GVAL=" << n.g_val
           << " ; HVAL=" << std::setprecision(4) << n.h_val
           << " ; #CONF=" << n.num_internal_conf << " ; PARENT=" << (n.parent)->loc
           << " ; IN_OPEN?" << std::boolalpha << n.in_openlist;
    else
        os << "LOC=" << n.loc << " ; TIMESTEP=" << n.timestep << " ; GVAL=" << n.g_val << " ; HVAL="
           << std::setprecision(4) << n.h_val
